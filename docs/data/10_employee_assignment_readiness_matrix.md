...
| Employee → manager | `puls_core.employee_reporting_lines` (`primary_manager`) | manager pill | tenant + admin read | manager approver | active primary line + manager `employment_status=active`; tie-break `starts_on DESC, created_at DESC` | **read-only** | hierarchy editor PR10.15+ |
