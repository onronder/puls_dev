...
> **Tarih:** 2026-06-10 · **Rev 14** (PR17.2G2B workflow evidence worker skeleton tamamlandı: disabled-by-default service package + private Storage read + server-side SHA-256 + mock/disabled provider adapter; production enqueue, Railway deploy ve OCR provider yok). Önceki: Rev 13 (PR17.2G2A OCR DB contract).
...
> ### 🔄 Rev 14 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B sonrası)
>
...
2. **Belge/evidence kenarı kısmi kapandı (HIGH→MEDIUM).** İzin eki, masraf fişi ve sözleşme dokümanı artık private storage + staging + RPC + form akışına bağlı; attached evidence görüntüleme/indirme UX'i PR17.2G1 ile açıldı. Masraf fişi OCR/extraction DB contract'ı PR17.2G2A ile eklendi; disabled worker skeleton ve server-side SHA-256 yolu PR17.2G2B ile eklendi. Üretim enqueue, insan incelemesi ve provider seçimi özellikle PR17.2G3-G4 konusudur.
...
**Ortalama readiness: ~62/100** (Rev14; 19 route). PR17.1 Core HR dilimleri ve PR17.2 notification/evidence upload/viewing/OCR-DB-contract/worker-skeleton dilimleri skorları belirgin yükseltti; STUB ailesi (performans-param/kariyer/eğitim/iş-değerleme) ve sözleşmelerin kalan ürün akışları hâlâ ortalamayı aşağı çekiyor — PR17.3'ün konusu.
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev14:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + disabled worker skeleton var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** workflow evidence worker skeleton ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+disabled worker tamam; production enqueue/human review/AI/e2e eksik | MEDIUM                                   | ~~80~~ **89**¹ |
...
- **Rev14 belge/evidence:** receipt-required durumda `EvidenceUploadField` görünür; `create_expense_claim_with_evidence` finalized evidence'i aynı transaction içinde `expense_receipts` kaydına bağlar. Attached receipt/document evidence artık private Storage RLS üzerinden kısa TTL signed URL ile görüntülenir. `expense_receipt_ocr_jobs/results/events` DB contract'ı ve disabled worker skeleton var; ancak production enqueue, real provider ve human review yoktur. **OCR teaser "SOON"** hâlâ fonksiyonel değildir; PR17.2G'nin kalan kısmı human review, production enqueue guard'ları ve en son vendor seçimi olarak fazlanmıştır.
...
| **R8**  | 🟡 **KISMİ (Rev14).** Belge/evidence upload storage/RPC, form akışı, finalize hardening, attached evidence viewing, expense receipt OCR DB contract ve disabled worker skeleton tamamlandı; human review/production enqueue/vendor evaluation açık.                                      | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing; PR17.2G2A service-role OCR queue/result/event DB contract; PR17.2G2B private storage read + server SHA-256 worker skeleton.       | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi ve insan review'u tamamlanmadan belge içeriği canonical veriye çevrilmez. |
...
> **Rev14 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A ve disabled worker skeleton G2B tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş human review, sonra production enqueue guard'ları ve en son vendor entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G3 — Human Review** (R8). Masraf fişi extraction yalnızca öneri/evidence üretmeli; structured parse/PDF text/duplicate/manual fallback pahalı OCR'dan önce gelmeli; canonical yazım insan onayı ve audit olmadan olmamalı.
...
1. **Belge upload PR17 scope'unda mı?** İzin belgesi / masraf fişi gerçek HR'da çoğu zaman zorunlu. Rev14 kararı: evet; PR17.2F1/F2/F3 ile storage/RLS/RPC/audit boundary, browser form akışı ve finalize storage-size hardening açıldı; PR17.2G1 ile attached evidence viewing açıldı; PR17.2G2A ile masraf fişi OCR/extraction DB contract'ı açıldı; PR17.2G2B ile disabled worker skeleton ve server-side content hash path'i eklendi. Malware scan veya server-verified hash claim'i browser/upload aşamasında yapılmaz; `sha256_client` client-declared metadata olarak kalır. PR17.2G'nin kalan kısmı human review, production enqueue guard'ları ve vendor seçimi kararlarını ayrı alt fazlara böler.
