...
> **Tarih:** 2026-06-10 · **Rev 13** (PR17.2G2A expense receipt OCR DB contract tamamlandı: provider-agnostic queue/result/event schema + service-role-only queue RPCs + rollback smoke; üretim enqueue, worker ve OCR provider yok). Önceki: Rev 12 (PR17.2G1 evidence viewing access).
...
> ### 🔄 Rev 13 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A sonrası)
>
...
> | **R8** belge/evidence upload                 | 17.2F1/F2/F3 + 17.2G1/G2A tamam; G2B-G4 açık | 🟡 **KISMİ**   | Private storage + staging + intent/finalize RPC + submit-with-evidence + leave/expense/contract UI tamam; attached evidence viewing açıldı; expense receipt OCR queue/result/event DB contract var; worker, production enqueue, human review ve vendor gates açık |
>
> **Net:** PR17.1 Core HR closed-loop **gerçekten kapandı** (audit + lifecycle + edit, hepsi server-validated/audited). PR17.2 notification platformu artık **connector-bağımsız canlı teslimat** yapar: workflow event trigger'ları notification ledger'a metadata-only kayıt düşer, mevcut producer orchestrator ise duplicate-safe backfill/reconcile yolu olarak kalır. PR17.2E bu sözleşmeyi `docs/data/17_2_e_workflow_e2e_reconcile_smoke.sql` ile kilitledi. PR17.2F1/F2 belge/evidence upload'ın storage/RPC ve browser form kenarını açtı; PR17.2F3 finalize aşamasını gerçek storage size metadata kontrolüyle sertleştirdi. PR17.2G1 attached evidence görüntüleme kenarını açtı: yetkili kullanıcılar private `workflow-evidence` Storage RLS sınırı içinde kısa TTL signed URL alır; public URL ve OCR yoktur. PR17.2G2A expense receipt OCR/extraction için DB contract'ı ekledi: service-role-only enqueue/claim/heartbeat/complete/recover RPC'leri, result ledger, duplicate suspicion ve immutable event log var; fakat üretim enqueue, worker, provider ve canonical write yoktur. G2B-G4 hâlâ worker skeleton, human review ve vendor evaluation olarak açıktır. Pahalı OCR provider'ı varsayılan yol değildir; structured parse/PDF text/duplicate/manual fallback önce gelir.

> #### PR17.2 alt faz kararı (Rev13)
>
> | Faz                                                    | Amaç                                                                                                                                                                                   | Durum         |
> | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
...
> | **17.2G2B — OCR Worker Skeleton**                       | Disabled-by-default worker: private storage read, server SHA-256, mock/disabled provider adapter, no external provider call                                                               | 🔴 Açık       |
...
**Tek cümlelik gerçek:** PULS'un _gerçek backend workflow motoru_ (RPC + RLS + audit trigger + approver resolver), _gerçek notification platformu_ ve _attached evidence görüntüleme kenarı_ artık HR workflow için bağlı; izin/masraf submit ve decision event'leri connector beklemeden Notification Center'a düşer. **AI context yüzeyi hâlâ HR mutation'larından beslenmiyor.**

Senin tanımladığın tam döngü — **oluştur → gönder → onayla → audit → notification → belge/evidence → AI context** — izin ve masraf için artık hardened evidence upload + authorized evidence viewing kenarına kadar ilerledi. Kalan sistemik açıklar **OCR/insan incelemesi**, **tarayıcı e2e kanıtı** ve **AI context** katmanıdır.

### Sayfa kategorileri

| Kategori                                                       | Sayfalar                                                                     | Durum                                                                                  |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **Gerçek workflow (backend + notification + evidence upload/viewing)** | izin, masraf                                                                 | OCR/human review, AI context ve browser e2e açık                                       |
...
2. **Belge/evidence kenarı kısmi kapandı (HIGH→MEDIUM).** İzin eki, masraf fişi ve sözleşme dokümanı artık private storage + staging + RPC + form akışına bağlı; attached evidence görüntüleme/indirme UX'i PR17.2G1 ile açıldı. Masraf fişi OCR/extraction DB contract'ı PR17.2G2A ile eklendi; worker, üretim enqueue ve insan incelemesi özellikle PR17.2G2B-G3 konusudur.
...
**Ortalama readiness: ~61/100** (Rev13; 19 route). PR17.1 Core HR dilimleri ve PR17.2 notification/evidence upload/viewing/OCR-DB-contract dilimleri skorları belirgin yükseltti; STUB ailesi (performans-param/kariyer/eğitim/iş-değerleme) ve sözleşmelerin kalan ürün akışları hâlâ ortalamayı aşağı çekiyor — PR17.3'ün konusu.
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev13:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB contract tamam; worker/human review/AI/e2e eksik | MEDIUM                                   | ~~80~~ **88**¹ |
...
- **Rev13 belge/evidence:** receipt-required durumda `EvidenceUploadField` görünür; `create_expense_claim_with_evidence` finalized evidence'i aynı transaction içinde `expense_receipts` kaydına bağlar. Attached receipt/document evidence artık private Storage RLS üzerinden kısa TTL signed URL ile görüntülenir. `expense_receipt_ocr_jobs/results/events` DB contract'ı var, ancak production enqueue ve worker yoktur. **OCR teaser "SOON"** hâlâ fonksiyonel değildir; PR17.2G'nin kalan kısmı disabled worker skeleton, sonra insan review, en son vendor seçimi olarak fazlanmıştır.
...
| **R8**  | 🟡 **KISMİ (Rev13).** Belge/evidence upload storage/RPC, form akışı, finalize hardening, attached evidence viewing ve expense receipt OCR DB contract tamamlandı; worker/human review/vendor evaluation açık.                                      | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing; PR17.2G2A service-role OCR queue/result/event DB contract.       | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi ve insan review'u tamamlanmadan belge içeriği canonical veriye çevrilmez. |
...
- ✅ **17.2G2A — OCR DB Contract tamamlandı:** yalnız expense receipts için provider-agnostic queue/result/event modeli ve service-role-only enqueue/claim/heartbeat/complete/recover RPC'leri. Üretim enqueue, worker, provider ve canonical write yok.
- **17.2G2B — OCR Worker Skeleton:** disabled-by-default worker, private storage read, server-side SHA-256, mock/disabled adapter; provider SDK/external call yok.
...
> **Rev13 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1 ve expense receipt OCR DB contract G2A tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş disabled worker skeleton, sonra human review, en son vendor/worker entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G2B/G3 — OCR Worker Skeleton + Human Review** (R8). Masraf fişi extraction yalnızca öneri/evidence üretmeli; structured parse/PDF text/duplicate/manual fallback pahalı OCR'dan önce gelmeli; canonical yazım insan onayı ve audit olmadan olmamalı.
...
1. **Belge upload PR17 scope'unda mı?** İzin belgesi / masraf fişi gerçek HR'da çoğu zaman zorunlu. Rev13 kararı: evet; PR17.2F1/F2/F3 ile storage/RLS/RPC/audit boundary, browser form akışı ve finalize storage-size hardening açıldı; PR17.2G1 ile attached evidence viewing açıldı; PR17.2G2A ile masraf fişi OCR/extraction DB contract'ı açıldı. Malware scan veya server-verified hash claim'i browser/upload aşamasında yapılmaz; `sha256_client` client-declared metadata olarak kalır. Server-side content hash G2B worker path'indedir. PR17.2G'nin kalan kısmı disabled worker skeleton, human review ve vendor seçimi kararlarını ayrı alt fazlara böler.
