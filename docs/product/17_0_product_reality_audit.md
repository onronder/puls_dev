...
> **Tarih:** 2026-06-11 · **Rev 19** (PR17.2G4B OCR queue resilience proof eklendi: worker heartbeat, stale lease recovery, retry, dead-letter ve receipt `ocr_status` projection tutarlılığı kanıtlandı; browser enqueue, Railway deploy, canonical expense write ve paid OCR/VLM provider hâlâ kapalı). Önceki: Rev 18 (PR17.2G4A OCR tenant posture/quota gate).
...
> ### 🔄 Rev 19 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A + G4/G4A/G4B sonrası)
>
...
**Ortalama readiness: ~63/100** (Rev19; 19 route). PR17.1 Core HR dilimleri ve PR17.2 notification/evidence upload/viewing/OCR-DB-contract/worker-skeleton/human-review/quota/resilience dilimleri skorları belirgin yükseltti; STUB ailesi (performans-param/kariyer/eğitim/iş-değerleme) ve sözleşmelerin kalan ürün akışları hâlâ ortalamayı aşağı çekiyor — PR17.3'ün konusu.
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev19:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + disabled worker skeleton + human review panel var; service-role OCR enqueue quota-gated, queue resilience kanıtlı, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** worker skeleton ✓; **Rev17:** `record_expense_receipt_ocr_review` ✓; **Rev18:** OCR tenant posture/quota gate ✓; **Rev19:** heartbeat/recover/dead-letter smoke ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+disabled worker+human review+quota gate+queue resilience tamam; browser enqueue/vendor/AI/e2e eksik | MEDIUM                                   | ~~80~~ **91**¹ |
...
| **R8**  | 🟡 **KISMİ (Rev19).** Belge/evidence upload storage/RPC, form akışı, finalize hardening, attached evidence viewing, expense receipt OCR DB contract, disabled worker skeleton, human review, self-review hardening, G4A quota gate ve G4B queue resilience tamamlandı; browser enqueue/provider integration açık. | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing; PR17.2G2A service-role OCR queue/result/event DB contract; PR17.2G2B private storage read + server SHA-256 worker skeleton; PR17.2G3 `record_expense_receipt_ocr_review` + masraf review panel; PR17.2G3A self-review block + negative actor smoke; PR17.2G4 vendor/VLM evaluation doc; PR17.2G4A tenant/global OCR posture + quota gate; PR17.2G4B worker heartbeat + recover/retry/dead-letter smoke. | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi human review kararı alabilir, queue lease/retry/dead-letter kanıtlıdır, ama browser enqueue, real provider ve canonical apply kapalıdır. |
...
> **Rev19 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, disabled worker skeleton G2B, human review G3, self-review hardening G3A, tenant OCR quota gate G4A ve queue resilience G4B tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş local benchmark harness ve en son vendor entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G4C — Local Extraction + Benchmark Harness** (R8). PDF text-layer/free-route extraction, Türkçe locale parse validation ve network-free benchmark harness eklenmeli; canonical write hâlâ ayrı apply kararı gerektirir.
...
1. **Belge upload PR17 scope'unda mı?** İzin belgesi / masraf fişi gerçek HR'da çoğu zaman zorunlu. Rev19 kararı: evet; PR17.2F1/F2/F3 ile storage/RLS/RPC/audit boundary, browser form akışı ve finalize storage-size hardening açıldı; PR17.2G1 ile attached evidence viewing açıldı; PR17.2G2A ile masraf fişi OCR/extraction DB contract'ı açıldı; PR17.2G2B ile disabled worker skeleton ve server-side content hash path'i eklendi; PR17.2G3 ile human review karar yüzeyi açıldı; PR17.2G4A ile service-role OCR enqueue tenant posture/quota gate arkasına alındı; PR17.2G4B ile queue heartbeat/recover/retry/dead-letter kanıtlandı. Malware scan veya server-verified hash claim'i browser/upload aşamasında yapılmaz; `sha256_client` client-declared metadata olarak kalır. PR17.2G'nin kalan kısmı local benchmark harness ve vendor seçimi kararlarını ayrı alt fazlara böler.
