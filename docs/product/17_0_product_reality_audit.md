...
> **Tarih:** 2026-06-13 · **Rev 22** (PR17.2G4C-B verify/parser hardening eklendi: PR17 verify script'leri sabit geçmiş Rev grep'leri yerine current Rev marker regex'ine geçti ve `KDV DAHIL TOPLAM` toplam satırının `tax_amount` false-positive üretmesi engellendi; browser enqueue, Railway deploy, canonical expense write ve paid OCR/VLM provider hâlâ kapalı). Önceki: Rev 21 (PR17.2G4C-A OCR local extraction hardening).
...
> ### 🔄 Rev 22 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A + G4/G4A/G4B/G4C/G4C-A/G4C-B sonrası)
>
...
> **Rev22 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, worker skeleton G2B, human review G3, self-review hardening G3A, tenant OCR quota gate G4A, queue resilience G4B, local benchmark harness G4C, G4C-A benchmark/worker hardening ve G4C-B verify/parser hardening tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki paid benchmark/production integration işleri ürün-hukuk-bütçe kararlarına bağlıdır. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.
