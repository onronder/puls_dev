...
> **Tarih:** 2026-06-10 · **Rev 12** (PR17.2G1 evidence viewing access tamamlandı: attached evidence için authorized read model + private Storage RLS üzerinden kısa TTL signed URL + compact view action). Önceki: Rev 11 (PR17.2G evidence viewing/OCR/human-review contract).
...
> ### 🔄 Rev 12 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1 sonrası)
>
...
> | **R8** belge/evidence upload                 | 17.2F1/F2/F3 + 17.2G1 tamam; G2-G4 açık | 🟡 **KISMİ**   | Private storage + staging + intent/finalize RPC + submit-with-evidence + leave/expense/contract UI tamam; attached evidence viewing private Storage RLS + short-lived signed URL ile açıldı; OCR/human review/vendor gates açık |
>
> **Net:** PR17.1 Core HR closed-loop **gerçekten kapandı** (audit + lifecycle + edit, hepsi server-validated/audited). PR17.2 notification platformu artık **connector-bağımsız canlı teslimat** yapar: workflow event trigger'ları notification ledger'a metadata-only kayıt düşer, mevcut producer orchestrator ise duplicate-safe backfill/reconcile yolu olarak kalır. PR17.2E bu sözleşmeyi `docs/data/17_2_e_workflow_e2e_reconcile_smoke.sql` ile kilitledi. PR17.2F1/F2 belge/evidence upload'ın storage/RPC ve browser form kenarını açtı; PR17.2F3 finalize aşamasını gerçek storage size metadata kontrolüyle sertleştirdi. PR17.2G1 attached evidence görüntüleme kenarını açtı: yetkili kullanıcılar private `workflow-evidence` Storage RLS sınırı içinde kısa TTL signed URL alır; public URL ve OCR yoktur. G2-G4 hâlâ provider-agnostic OCR job/result, human review ve vendor evaluation olarak açıktır. Pahalı OCR provider'ı varsayılan yol değildir; structured parse/PDF text/duplicate/manual fallback önce gelir.

> #### PR17.2 alt faz kararı (Rev12)
>
> | Faz                                                    | Amaç                                                                                                                                                                                   | Durum         |
> | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
...
2. **Belge/evidence kenarı kısmi kapandı (HIGH→MEDIUM).** İzin eki, masraf fişi ve sözleşme dokümanı artık private storage + staging + RPC + form akışına bağlı; attached evidence görüntüleme/indirme UX'i PR17.2G1 ile açıldı. OCR/insan incelemesi ise özellikle masraf fişi için PR17.2G2-G3 konusudur.
...
**Ortalama readiness: ~61/100** (Rev12; 19 route). PR17.1 Core HR dilimleri ve PR17.2 notification/evidence upload/viewing dilimleri skorları belirgin yükseltti; STUB ailesi (performans-param/kariyer/eğitim/iş-değerleme) ve sözleşmelerin kalan ürün akışları hâlâ ortalamayı aşağı çekiyor — PR17.3'ün konusu.
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev12:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓ | 🟡 Backend+notification+evidence upload/view tamam; OCR/AI/e2e eksik | MEDIUM                                   | ~~80~~ **88**¹ |
...
- **Rev12 belge/evidence:** receipt-required durumda `EvidenceUploadField` görünür; `create_expense_claim_with_evidence` finalized evidence'i aynı transaction içinde `expense_receipts` kaydına bağlar. Attached receipt/document evidence artık private Storage RLS üzerinden kısa TTL signed URL ile görüntülenir. **OCR teaser "SOON"** hâlâ fonksiyonel değildir; PR17.2G'nin kalan kısmı provider-agnostic OCR/job, sonra insan review, en son vendor seçimi olarak fazlanmıştır.
...
| **R8**  | 🟡 **KISMİ (Rev12).** Belge/evidence upload storage/RPC, form akışı, finalize hardening ve attached evidence viewing tamamlandı; OCR/human review/vendor evaluation açık.                                      | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing.       | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi ve insan review'u tamamlanmadan belge içeriği canonical veriye çevrilmez. |
...
> **Rev12 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3 ve evidence viewing G1 tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş provider-agnostic extraction contract, sonra human review, en son vendor/worker entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G2/G3 — OCR Contract + Human Review** (R8). Masraf fişi extraction yalnızca öneri/evidence üretmeli; structured parse/PDF text/duplicate/manual fallback pahalı OCR'dan önce gelmeli; canonical yazım insan onayı ve audit olmadan olmamalı.
...
1. **Belge upload PR17 scope'unda mı?** İzin belgesi / masraf fişi gerçek HR'da çoğu zaman zorunlu. Rev12 kararı: evet; PR17.2F1/F2/F3 ile storage/RLS/RPC/audit boundary, browser form akışı ve finalize storage-size hardening açıldı; PR17.2G1 ile attached evidence viewing açıldı. Malware scan veya server-verified hash claim'i yapılmaz; `sha256_client` client-declared metadata olarak kalır. PR17.2G'nin kalan kısmı provider-agnostic OCR/structured extraction, human review ve vendor seçimi kararlarını ayrı alt fazlara böler.
