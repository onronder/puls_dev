...
> **Tarih:** 2026-06-12 · **Rev 21** (PR17.2G4C-A OCR local extraction hardening eklendi: gerçek PDF-byte benchmark coverage, text fixture ayrımı, adversarial total/amount fixture'ları, `mean_field_accuracy` verify gate'i, provider-class mismatch guard, non-overwriting heartbeat ve transient retry sınıflandırması tamamlandı; browser enqueue, Railway deploy, canonical expense write ve paid OCR/VLM provider hâlâ kapalı). Önceki: Rev 20 (PR17.2G4C OCR local extraction benchmark).
...
> ### 🔄 Rev 21 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A + G4/G4A/G4B/G4C/G4C-A sonrası)
>
...
> **Rev21 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, worker skeleton G2B, human review G3, self-review hardening G3A, tenant OCR quota gate G4A, queue resilience G4B, local benchmark harness G4C ve G4C-A benchmark/worker hardening tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki paid benchmark/production integration işleri ürün-hukuk-bütçe kararlarına bağlıdır. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.
