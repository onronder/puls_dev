...
> **Tarih:** 2026-06-10 · **Rev 16** (PR17.2G3A OCR review hardening tamamlandı: self-review server-side bloklandı ve rollback smoke unauthorized/requester negatif aktörleri kapsar; canonical expense write, production enqueue, Railway deploy ve OCR provider yok). Önceki: Rev 15 (PR17.2G3 human review).
...
> ### 🔄 Rev 16 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A sonrası)
>
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev16:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + disabled worker skeleton + human review panel var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** worker skeleton ✓; **Rev16:** `record_expense_receipt_ocr_review` ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+disabled worker+human review tamam; production enqueue/vendor/AI/e2e eksik | MEDIUM                                   | ~~80~~ **91**¹ |
...
| **R8**  | 🟡 **KISMİ (Rev16).** Belge/evidence upload storage/RPC, form akışı, finalize hardening, attached evidence viewing, expense receipt OCR DB contract, disabled worker skeleton, human review ve self-review hardening tamamlandı; production enqueue/vendor evaluation açık.                                      | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing; PR17.2G2A service-role OCR queue/result/event DB contract; PR17.2G2B private storage read + server SHA-256 worker skeleton; PR17.2G3 `record_expense_receipt_ocr_review` + masraf review panel; PR17.2G3A self-review block + negative actor smoke.       | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi human review kararı alabilir, ama production OCR ve canonical apply kapalıdır. |
...
> **Rev16 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, disabled worker skeleton G2B, human review G3 ve self-review hardening G3A tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş production enqueue guard'ları ve en son vendor entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.
