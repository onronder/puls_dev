...
> **Tarih:** 2026-06-15 · **Rev 23** (PR17.2G4D paid benchmark runbook/decision protocol eklendi: route-specific COGS, structured-output prompt posture, untrusted document/prompt-injection guard, screening/finalist statistical gates, redacted result shape ve review-savings economics kilitlendi; live provider call, SDK, credential, browser enqueue, Railway deploy, canonical expense write ve paid OCR/VLM provider hâlâ kapalı). Önceki: Rev 22 (PR17.2G4C-B verify/parser hardening).
...
> ### 🔄 Rev 23 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A + G4/G4A/G4B/G4C/G4C-A/G4C-B/G4D sonrası)
>
...
> | **R8** belge/evidence upload                 | 17.2F1/F2/F3 + 17.2G1/G2A/G2B/G3/G3A tamam; G4 karar dokümanı + G4A quota gate + G4B queue resilience + G4C local benchmark + G4C-A/B hardening + G4D benchmark runbook eklendi | 🟡 **KISMİ**   | Private storage + staging + intent/finalize RPC + submit-with-evidence + leave/expense/contract UI tamam; attached evidence viewing açıldı; expense receipt OCR queue/result/event DB contract var; worker skeleton server-side hash hesaplayabilir ve lease heartbeat atar; human review decision UI/RPC tamam; self-review ve unauthorized actor negatif smoke kapalı; G4 vendor/VLM evaluation dokümanı VLM benchmark yönünü tanımlar; G4A service-role enqueue'u tenant posture, quota, spend cap, provider allowlist ve file/page limitleriyle kapılar; G4B stale lease recover/retry/dead-letter ve receipt projection tutarlılığını kanıtlar; G4C `pdf_text` free-route extraction ve sentetik benchmark harness ekler; G4C-A gerçek PDF-byte coverage, adversarial amount/total guard, accuracy gate, provider-class mismatch, non-overwriting heartbeat ve transient retry sınıflandırmasını kanıtlar; G4C-B current Rev marker verify guard'ı ve `KDV DAHIL TOPLAM` tax false-positive düzeltmesini ekler; G4D paid benchmark runbook'u route-specific COGS, prompt-injection guard, statistical decision rule, redacted result template ve review-savings economics'i zorunlu kılar; browser enqueue ve provider entegrasyonu hâlâ kapalı |
>
> **Net:** PR17.1 Core HR closed-loop **gerçekten kapandı** (audit + lifecycle + edit, hepsi server-validated/audited). PR17.2 notification platformu artık **connector-bağımsız canlı teslimat** yapar: workflow event trigger'ları notification ledger'a metadata-only kayıt düşer, mevcut producer orchestrator ise duplicate-safe backfill/reconcile yolu olarak kalır. PR17.2E bu sözleşmeyi `docs/data/17_2_e_workflow_e2e_reconcile_smoke.sql` ile kilitledi. PR17.2F1/F2 belge/evidence upload'ın storage/RPC ve browser form kenarını açtı; PR17.2F3 finalize aşamasını gerçek storage size metadata kontrolüyle sertleştirdi. PR17.2G1 attached evidence görüntüleme kenarını açtı: yetkili kullanıcılar private `workflow-evidence` Storage RLS sınırı içinde kısa TTL signed URL alır; public URL yoktur. PR17.2G2A expense receipt OCR/extraction için DB contract'ı ekledi: service-role-only enqueue/claim/heartbeat/complete/recover RPC'leri, result ledger, duplicate suspicion ve immutable event log var. PR17.2G2B ayrı worker skeleton'ı ekledi: private storage object indirir, server-side SHA-256 hesaplar ve disabled/mock provider result'ını G2A RPC'leriyle tamamlar. PR17.2G3 human review boundary'yi ekledi; PR17.2G3A bu boundary'yi self-review blokajı ve unauthorized/requester negatif smoke ile sertleştirdi. PR17.2G4 karar dokümanı pahalı vertical OCR yerine VLM/document-to-JSON benchmark yönünü, provider enum mapping'ini, route coverage ölçümünü ve quota/spend/KVKK kapılarını netleştirdi. PR17.2G4A tenant/global OCR posture tablolarını ve enqueue quota gate'ini ekledi; service-role enqueue artık default kapalıdır ve ancak tenant flag, quota, spend cap, provider allowlist, file/page limitleri geçerse iş kuyruğa alınır. PR17.2G4B queue resilience borcunu kapattı: worker heartbeat atar, stale lease recovery retry/dead-letter davranışını ve receipt `ocr_status` projection tutarlılığını rollback smoke kanıtlar. PR17.2G4C free-route ölçüm zeminini açtı: `pdf_text` local extraction, Türkçe amount/date/currency parser, adversarial sentetik fixture ve network-free benchmark runner var. PR17.2G4C-A bu zemini G4D öncesi sertleştirdi: benchmark artık gerçek PDF-byte fixture'larını direct text fixture'lardan ayırır, `mean_field_accuracy` ve adversarial totals verify-gatedir, Türkçe toplam parser'ı English `TOTAL`/`1,234.56` tuzaklarına yanlış değer üretmez, worker provider-class mismatch'i güvenli fail eder, heartbeat mevcut job context'ini overwrite etmez ve transient worker hataları `retrying` olur. PR17.2G4C-B verify disiplinini sabit geçmiş Rev grep'lerinden current Rev marker regex'ine taşıdı ve `KDV DAHIL TOPLAM` toplam satırının `tax_amount` false-positive üretmesini engelledi. PR17.2G4D paid benchmark runbook/decision protocol'ünü ekledi: gerçek run başlamadan dataset/KVKK, bütçe, region/residency, credential ve result-handling kapıları gerekir; provider sonuçları route-specific COGS, structured output, untrusted-document prompt posture, screening/finalist statistical gates, redacted aggregate result ve review-savings economics ile değerlendirilecektir. Kalan G4 işi bu kapılar açılırsa ops-only benchmark run ve sonrasında ayrı G4E production integration'dır. Pahalı OCR provider'ı varsayılan yol değildir; structured parse/PDF text/duplicate/manual fallback önce gelir.

> #### PR17.2 alt faz kararı (Rev22)
>
> | Faz                                                    | Amaç                                                                                                                                                                                                         | Durum                                                |
> | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
...
> | **17.2G4C-A — OCR Local Extraction Hardening**         | Gerçek PDF-byte benchmark coverage, direct text fixture ayrımı, adversarial total/amount guard, accuracy gate, provider-class matching, non-overwriting heartbeat ve transient retry sınıflandırmasını ekle  | ✅ Tamamlandı                                        |
> | **17.2G4C-B — OCR Verify & Parser Hardening**          | PR17 verify script'lerini current Rev marker regex'ine geçir; `KDV DAHIL TOPLAM` toplam satırının `tax_amount` false-positive üretmesini engelle                                                             | ✅ Tamamlandı                                        |
> | **17.2G4D — OCR Paid Benchmark Runbook**               | Paid provider benchmark'ı için KVKK/bütçe/region/credential kapıları, route-specific COGS, structured-output prompt posture, statistical decision rule, redacted result template ve review-savings economics | ✅ Tamamlandı                                        |
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev21:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + worker skeleton + human review panel var; service-role OCR enqueue quota-gated, queue resilience kanıtlı, local `pdf_text` benchmark hardening var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** worker skeleton ✓; **Rev17:** `record_expense_receipt_ocr_review` ✓; **Rev18:** OCR tenant posture/quota gate ✓; **Rev19:** heartbeat/recover/dead-letter smoke ✓; **Rev20:** local `pdf_text` benchmark ✓; **Rev21:** PDF-byte benchmark + adversarial/accuracy worker hardening ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+worker+human review+quota gate+queue resilience+local benchmark hardening tamam; browser enqueue/vendor/AI/e2e eksik | MEDIUM                                   | ~~80~~ **91**¹ |
...
- ✅ **17.2G4C-A — OCR Local Extraction Hardening tamamlandı:** benchmark gerçek PDF-byte fixture'larını direct text fixture'lardan ayırır; adversarial `TOTAL 0,00`, ambiguous `1,234.56`, columnar Turkish totals, `mean_field_accuracy`, provider-class mismatch, non-overwriting heartbeat ve transient retry sınıflandırması verify kapsamına alındı.
- ✅ **17.2G4C-B — OCR Verify & Parser Hardening tamamlandı:** PR17 verify script'leri sabit geçmiş Rev grep'leri yerine current Rev marker regex'i kullanır; `KDV DAHIL TOPLAM` toplam satırı artık `tax_amount` false-positive üretmez.
- ✅ **17.2G4D — OCR Paid Benchmark Runbook tamamlandı:** paid provider benchmark'ı başlamadan dataset/KVKK, bütçe, region/residency, credential ve result-handling kapıları zorunludur; route-specific COGS, structured-output prompt posture, untrusted-document guard, statistical decision rule, redacted result template ve review-savings economics verify kapsamına alındı. Live provider call, SDK, credential, enqueue, deployment ve canonical write yoktur.
...
> **Rev23 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, worker skeleton G2B, human review G3, self-review hardening G3A, tenant OCR quota gate G4A, queue resilience G4B, local benchmark harness G4C, G4C-A benchmark/worker hardening, G4C-B verify/parser hardening ve G4D paid benchmark runbook tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki ops-only paid benchmark run/production integration işleri ürün-hukuk-bütçe kararlarına bağlıdır. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G4D-Ops — Paid Provider Benchmark Run** (R8). Gerçek/sentetik benchmark veri seti KVKK onayı, bütçe/quota default'ları, region/residency kararı, credential handling ve redacted result handling bağlanmadan paid provider benchmark koşulmamalı; canonical write hâlâ ayrı G4E apply/integration kararı gerektirir.
