...
> **Tarih:** 2026-06-10 · **Rev 15** (PR17.2G3 expense receipt OCR human review tamamlandı: reviewer karar RPC'si, audit-backed review metadata ve masraf onay UI paneli; canonical expense write, production enqueue, Railway deploy ve OCR provider yok). Önceki: Rev 14 (PR17.2G2B worker skeleton).
...
> ### 🔄 Rev 15 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3 sonrası)
>
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev15:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + disabled worker skeleton + human review panel var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** worker skeleton ✓; **Rev15:** `record_expense_receipt_ocr_review` ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+disabled worker+human review tamam; production enqueue/vendor/AI/e2e eksik | MEDIUM                                   | ~~80~~ **91**¹ |
...
> **Rev15 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, disabled worker skeleton G2B ve human review G3 tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş production enqueue guard'ları ve en son vendor entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G4 — Vendor Evaluation & Worker Integration** (R8). Production OCR enqueue yalnız tenant flag, quota/cost, KVKK/GDPR, data residency, retention ve gerçek Türkçe belge benchmark'ı netleşince açılmalı; canonical write hâlâ ayrı apply kararı gerektirir.
