...
> **Tarih:** 2026-06-11 · **Rev 17** (PR17.2G4 OCR vendor/VLM extraction evaluation dokümanı eklendi: benchmark + quota gate yönü netleşti; canonical expense write, production enqueue, Railway deploy ve paid OCR/VLM provider hâlâ kapalı). Önceki: Rev 16 (PR17.2G3A OCR review hardening).
...
> ### 🔄 Rev 17 güncel durum (PR17.1A-D + PR17.2A-F3 + PR17.2G1/G2A/G2B/G3/G3A + G4 karar dokümanı sonrası)
>
...
| 8   | `/masraf`                   | Employee + Manager | **Tam workflow** + server receipt policy; **Rev17:** gerekli fiş/belge upload + attached evidence görüntüleme açık, OCR DB contract + disabled worker skeleton + human review panel var, OCR vendor seçilmedi | `create/decide` RPC; `PULS_RECEIPT_REQUIRED` ✓; **Rev5:** same-transaction notification dispatch ✓; **Rev9:** `create_expense_claim_with_evidence` ✓; **Rev12:** Storage RLS signed URL ✓; **Rev13:** `expense_receipt_ocr_*` DB contract ✓; **Rev14:** worker skeleton ✓; **Rev17:** `record_expense_receipt_ocr_review` ✓ | 🟡 Backend+notification+evidence upload/view/OCR DB+disabled worker+human review tamam; production enqueue/vendor/AI/e2e eksik | MEDIUM                                   | ~~80~~ **91**¹ |
...
- **Rev17 belge/evidence:** receipt-required durumda `EvidenceUploadField` görünür; `create_expense_claim_with_evidence` finalized evidence'i aynı transaction içinde `expense_receipts` kaydına bağlar. Attached receipt/document evidence artık private Storage RLS üzerinden kısa TTL signed URL ile görüntülenir. `expense_receipt_ocr_jobs/results/events` DB contract'ı, disabled worker skeleton ve `record_expense_receipt_ocr_review` human-review RPC/UI paneli var; ancak production enqueue, real provider, canonical OCR apply ve AI context yoktur. **OCR teaser "SOON"** form içinde hâlâ production OCR sözü vermez; canlı panel yalnız mevcut OCR result varsa görünür.
...
| **R8**  | 🟡 **KISMİ (Rev17).** Belge/evidence upload storage/RPC, form akışı, finalize hardening, attached evidence viewing, expense receipt OCR DB contract, disabled worker skeleton, human review ve self-review hardening tamamlandı; G4 karar dokümanı eklendi, production enqueue/provider integration açık. | **HIGH→MEDIUM** | PR17.2F1 private bucket + staging + intent/finalize/attach RPC; PR17.2F2 `create_*_with_evidence` + izin/masraf/sözleşme UI; PR17.2F3 actual storage size metadata verification; PR17.2G1 private Storage RLS signed URL viewing; PR17.2G2A service-role OCR queue/result/event DB contract; PR17.2G2B private storage read + server SHA-256 worker skeleton; PR17.2G3 `record_expense_receipt_ocr_review` + masraf review panel; PR17.2G3A self-review block + negative actor smoke; PR17.2G4 vendor/VLM evaluation doc. | Gerçek HR'da masraf fişi/izin belgesi/sözleşme dokümanı artık yüklenebilir ve yetkili kullanıcı tarafından görülebilir; OCR önerisi human review kararı alabilir, ama production OCR ve canonical apply kapalıdır. |
...
> **Rev17 not:** Notification taxonomy, producer, preferences UI, connector-bağımsız dispatch, DB-boundary e2e/reconcile guard, audit trigger'ları, Core HR edit işleri, evidence upload F1/F2/F3, evidence viewing G1, expense receipt OCR DB contract G2A, disabled worker skeleton G2B, human review G3 ve self-review hardening G3A tamamlandı. PR17.2G artık tek parça "OCR yap" işi değildir; sıradaki iş production enqueue guard'ları ve en son vendor entegrasyonu olarak fazlandı. Aşağıdaki liste artık "workflow ürünü bitti" varsaymadan kalan productization kenarlarını sıralar. Orijinal liste tarihsel referans için §5-arşiv'de.

1. **PR17.2G4A — Tenant OCR Posture & Quota Gate** (R8). Production OCR enqueue yalnız tenant flag, quota/cost, KVKK/GDPR, data residency, retention ve gerçek Türkçe belge benchmark'ı netleşince açılmalı; canonical write hâlâ ayrı apply kararı gerektirir.
...
1. **Belge upload PR17 scope'unda mı?** İzin belgesi / masraf fişi gerçek HR'da çoğu zaman zorunlu. Rev17 kararı: evet; PR17.2F1/F2/F3 ile storage/RLS/RPC/audit boundary, browser form akışı ve finalize storage-size hardening açıldı; PR17.2G1 ile attached evidence viewing açıldı; PR17.2G2A ile masraf fişi OCR/extraction DB contract'ı açıldı; PR17.2G2B ile disabled worker skeleton ve server-side content hash path'i eklendi; PR17.2G3 ile human review karar yüzeyi açıldı. Malware scan veya server-verified hash claim'i browser/upload aşamasında yapılmaz; `sha256_client` client-declared metadata olarak kalır. PR17.2G'nin kalan kısmı production enqueue guard'ları ve vendor seçimi kararlarını ayrı alt fazlara böler.
