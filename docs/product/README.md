...
PR17.2A-E closes workflow notification delivery plus the database-boundary e2e/reconcile guard. PR17.2F1-F3 closes the upload/finalize boundary. PR17.2G1-G3A adds evidence viewing, OCR processing contracts, a disabled worker skeleton, and human review. PR17.2G4 adds the vendor/VLM extraction decision document and benchmark gate; G4A adds the tenant OCR posture and quota gate; G4B proves queue heartbeat/recover/dead-letter resilience; G4C adds local PDF-text extraction and a network-free benchmark harness; G4C-A hardens parser accuracy, real PDF-byte benchmark coverage, adversarial fixtures, provider-class matching, non-overwriting heartbeats, and transient retry classification; G4C-B hardens audit Rev verification and the `KDV DAHIL TOPLAM` tax false-positive edge; G4D adds the paid benchmark runbook and decision protocol. It still does not complete production OCR: browser enqueue, paid provider integration, richer browser journeys, and AI context feed remain intentionally separate slices.
...
| [PR17.2G4C OCR Local Extraction Benchmark](./17_2_g4c_expense_receipt_ocr_local_extraction_benchmark.md) | Completed local `pdf_text` extraction route, Turkish amount/date parser, synthetic fixture contract, network-free benchmark runner, and G4C-A/G4C-B hardening; no paid provider or customer dataset.              |
| [PR17.2G4D OCR Paid Benchmark Runbook](./17_2_g4d_expense_receipt_ocr_paid_benchmark_runbook.md)         | Completed paid-provider benchmark runbook and decision protocol: route-specific COGS, structured-output prompt posture, statistical gates, redacted result shape, and review-savings economics; no provider call. |
...
Reference: [17_0_product_reality_audit.md](./17_0_product_reality_audit.md) Rev 23.
