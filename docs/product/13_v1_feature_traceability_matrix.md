...
| ERP setup/readiness | specs/05 §4.16 | `/erp` | `setup/erp.ts` | `puls_integration.erp_*`, `source_namespaces`, `entity_identity_map` | Provider metadata + canonical mapping — `required seeded` (Canias inactive first provider) | ERP mapping CSV future | Setup coach | `db_backed_demo_required` | **PR14.1:** provider-agnostic connector preflight. **PR14.2:** no-connector onboarding state before metadata exists. Runtime connector future | PR14.2 |
