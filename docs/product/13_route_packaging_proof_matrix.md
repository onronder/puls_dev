...
| `/erp` | Connector preflight readiness | Provider metadata + mappings | source_namespaces proof in `07` | hr_admin | `source: real` | PR14.2 adds no-connector onboarding; PR14.3 adds provider draft review; PR14.4 adds PULS Connector Lab rollout posture; no runtime |
...
- **`/erp`** — connector preflight readiness (PR14.1), no-connector onboarding empty state (PR14.2), provider draft review (PR14.3), and PULS Connector Lab rollout posture (PR14.4); canonical mapping + unified namespace; Canias is the first provider, not the product abstraction; no runtime sync.
