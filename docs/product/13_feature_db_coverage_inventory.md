...
- **20 PR12 read-model routes** represented; matrix preserves `/performans` overview + cycles split, `/menu` shell row, and **cost center readiness** as its own capability on `/masraf-kategorileri` (**22 capability rows** total).
...
| Expense categories | `/masraf-kategorileri` | `setup/expense-categories.ts` | `workflow/policies.ts` | `puls_workflow.expense_categories`, `expense_category_lifecycle_events`; RPC deactivate/restore | `puls_workflow` | Real CRUD; overview demo fallback | `required seeded` | Categories, policy bindings | `fetchDemoExpenseCategoriesOverview` — **P1_replace_with_db_seed** | Setup coach | PR13.3 |
| Cost center readiness | `/masraf-kategorileri` | `setup/cost-center-readiness.ts` | — | `puls_core.cost_centers`, `puls_integration.source_namespaces`, `entity_identity_map`, `approval_policies`, `approval_policy_steps`, `expense_categories` (routing) | `puls_core`, `puls_integration`, `puls_workflow` | Separate WithMeta query on route; real readiness when seeded; demo fallback | `required seeded` | Cost centers + ERP/import mapping readiness for expense routing | `fetchDemoCostCenterReadinessOverview` — **P1_replace_with_db_seed** | Setup coach | PR13.3 |
...
| Cost center + ERP mapping seed | Cost center readiness on `/masraf-kategorileri` | PR13.3 |
