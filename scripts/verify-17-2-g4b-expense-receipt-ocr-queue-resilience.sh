...
require_pattern "$WORKER" "withLeaseHeartbeat"
require_pattern "$WORKER" "p_lease_seconds: config.leaseSeconds"
require_pattern "$WORKER" "p_safe_error_context: {}"
require_pattern "$WORKER" "resolveFailureStatus"
require_pattern "$WORKER" "ocr_provider_class_mismatch"
...
require_pattern "$WORKER_TEST" "provider_class: 'pdf_text'"
require_pattern "$WORKER_TEST" "status: 'retrying'"
require_pattern "$WORKER_TEST" "p_safe_error_context: {}"
...
require_pattern "$SMOKE" "heartbeat did not extend the active lease"
