...
function ApprovalsTab({ approvals, locale, t }: ApprovalsTabProps) {
  function handleDemoApprove(name: string) {
    toast.info(t('leaveSetup.approval.demoApproveTitle'), {
      description: t('leaveSetup.approval.demoApproveDesc', { name }),
    })
  }

  function handleDemoReject(name: string) {
    toast.info(t('leaveSetup.approval.demoRejectTitle'), {
      description: t('leaveSetup.approval.demoRejectDesc', { name }),
    })
  }
            ...
            >
              ...
              <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto">
                <Button
                  ...
                  onClick={() => handleDemoReject(approval.employeeName)}
                >
                ...
                <Button
                  ...
                  size="sm"
                  className="min-h-11 flex-1 sm:flex-none"
                  onClick={() => handleDemoApprove(approval.employeeName)}
                >
...
function LeaveFormSheet({
  ...
  const balanceAfter =
    leaveType === 'annual' ? annualRemaining - requestedDays : annualRemaining
  ...
  function resetForm() {
    ...
    setHalfDay(false)
    setDelegate('')
    setApprover(data?.approvers[0]?.id ?? '')
    setDescription('')
    setErrors({})
  }
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const approverName = data?.approvers.find((item) => item.id === approver)?.name
  ...
  const selectedTypeLabel =
    data?.leaveTypes.find((item) => item.id === leaveType)?.label ?? t('leave.types.annual')
  const balanceIsNegative = leaveType === 'annual' && balanceAfter < 0
  ...
  return (
    ...
    >
      <form id="new-leave-form" className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(data?.leaveTypes ?? []).map((type) => (
              <button
                ...
                aria-pressed={leaveType === type.id}
                className={cn(
        ...
        <FormField label={t('leaveSetup.form.approver')} hint={t('leaveSetup.form.approverHint')}>
          <select
            value={approver}
