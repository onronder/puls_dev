-- PR16.9.6 notification action routing and producer orchestration.
-- Adds a service-role app-wide producer orchestrator that the Railway
-- connector worker can call after real connector jobs. The orchestrator keeps
-- connector producer dedupe keys unchanged and does not open authenticated
-- table writes, external delivery, provider calls, source writeback,
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    'pr16.9.6-notification-action-routing-v1'::TEXT AS contract_version,
...
COMMENT ON FUNCTION puls_app.run_app_notification_producers(INTEGER, UUID) IS
  'PR16.9.6 service-role app-wide notification producer orchestrator. Currently runs the connector/runtime producer without opening browser writes, external delivery, provider calls, source writeback, or sensitive readback.';

COMMENT ON FUNCTION puls_app.get_notification_center_bootstrap_status() IS
  'PR16.9.6 app-wide Notification Center status RPC. Action routing, issue CSV export, and worker-driven producer orchestration are enabled; external delivery remains closed.';
